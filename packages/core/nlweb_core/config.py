...
class RetrievalProviderConfig:
    ...
    auth_method: Optional[str] = None  # Authentication method (api_key, azure_ad)
    import_path: Optional[str] = None
    class_name: Optional[str] = None
@dataclass
class SSLConfig:
...
class AppConfig:
    ...
    def __init__(self):
        ...
        # Check if using new unified config.yaml format
        unified_config_path = os.path.join(self.config_directory, 'config.yaml')
        if os.path.exists(unified_config_path):
            self._load_unified_config(unified_config_path)
        elif os.path.exists(os.path.join(self.config_directory, 'config_llm.yaml')):
            # Fall back to old multi-file format only if files exist
        ...
        # If no config files exist, just use defaults (will be set by init() later)

    def _load_unified_config(self, config_path: str):
        """
        Load configuration from a single unified config.yaml file.
        ...
        if 'high-llm-model' in config or 'low-llm-model' in config or 'scoring-llm-model' in config:
            ...
            self.preferred_llm_endpoint = provider_name
            self.llm_endpoints = {
                provider_name: LLMProviderConfig(
                    llm_type=self._get_config_value(llm_cfg.get('llm_type', provider_name)),
                    api_key=self._get_config_value(llm_cfg.get('api_key_env')),
                    models=models,
                    endpoint=self._get_config_value(llm_cfg.get('endpoint_env')),
                    api_version=self._get_config_value(llm_cfg.get('api_version')),
                    auth_method=self._get_config_value(llm_cfg.get('auth_method'), 'api_key'),
                    import_path=self._get_config_value(llm_cfg.get('import_path')),
                    class_name=self._get_config_value(llm_cfg.get('class_name'))
                )
            }
        else:
            self.preferred_llm_endpoint = None
        ...
        # Load embedding config from unified file
        if 'embedding' in config:
            emb_cfg = config['embedding']
            provider_name = emb_cfg.get('provider', 'default')

            self.preferred_embedding_provider = provider_name
            self.embedding_providers = {
                provider_name: EmbeddingProviderConfig(
                    api_key=self._get_config_value(emb_cfg.get('api_key_env')),
                    endpoint=self._get_config_value(emb_cfg.get('endpoint_env')),
                    api_version=self._get_config_value(emb_cfg.get('api_version')),
                    model=self._get_config_value(emb_cfg.get('model')),
                    config=self._get_config_value(emb_cfg.get('config')),
                    auth_method=self._get_config_value(emb_cfg.get('auth_method'), 'api_key'),
                    import_path=self._get_config_value(emb_cfg.get('import_path')),
                    class_name=self._get_config_value(emb_cfg.get('class_name'))
                )
            }
        else:
            self.preferred_embedding_provider = None
        ...
        # Load retrieval config from unified file
        if 'retrieval' in config:
            ...
            self.retrieval_endpoints = {
                provider_name: RetrievalProviderConfig(
                    api_key=self._get_config_value(ret_cfg.get('api_key_env')),
                    api_key_env=ret_cfg.get('api_key_env'),
                    api_endpoint=self._get_config_value(ret_cfg.get('api_endpoint_env')),
                    api_endpoint_env=ret_cfg.get('api_endpoint_env'),
                    database_path=self._get_config_value(ret_cfg.get('database_path')),
                    index_name=self._get_config_value(ret_cfg.get('index_name')),
                    db_type=self._get_config_value(ret_cfg.get('db_type', provider_name)),
                    enabled=ret_cfg.get('enabled', True),
                    use_knn=ret_cfg.get('use_knn'),
                    vector_type=ret_cfg.get('vector_type'),
                    auth_method=self._get_config_value(ret_cfg.get('auth_method'), 'api_key'),
                    import_path=self._get_config_value(ret_cfg.get('import_path')),
                    class_name=self._get_config_value(ret_cfg.get('class_name'))
                )
            }
            self.write_endpoint = provider_name
        else:
            self.retrieval_endpoints = {}
            self.write_endpoint = None
    ...
    def _get_config_directory(self) -> str:
        ...
        if not config_dir:
            # Default: config folder at the same level as this file (config.py)
            current_dir = os.path.dirname(os.path.abspath(__file__))  # nlweb_core directory
